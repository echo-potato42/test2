...
const COLUMN_CONFIG = {
  ...
  inProgress: { title: 'In Progress', emoji: '', wip: 1 },
  done: { title: 'Done', emoji: '' },
}

export function renderColumn(status, tasks, { onEdit, onAddSubtask }) {
  ...
  if (tasks.length === 0) {
    ...
    for (const task of tasks) {
      body.appendChild(renderTaskCard(task, { onEdit, onAddSubtask }))
    }
  }
